...
- `start_fixture` is the orchestrated fixture capture: input timeline + per-frame samples + CSV on the host disk. The CSV is an MCP artifact: the result returns `path` (host), `wsl_path` (WSL `/mnt/...`, Windows hosts only) and `resource` (bizhawk://) — read it directly from disk, never re-capture via Lua. It frame-advances (pausing/unpausing like `frame_advance`) and samples after each frame; max 600 frames. `read_struct` reads relative-offset fields from a base/symbol in one pass.
...
- Watchers/breakpoints are **polling-based** (`watch_*`, `wait_until`, `watch_change`) OR **real watchpoints** (`watchpoint_*`) — the latter use `IDebuggable.MemoryCallbacks` reached via reflection on `EmulationApi.DebuggableCore` (private `[OptionalService]`). **Only the Genesis gpgx waterbox core exposes memory callbacks** (single scope `M68K BUS` — no Z80 callbacks exist); every other core returns a clear `INVALID_PARAMS` error. No per-instruction stepping exists (`CanStep` is false on gpgx) — `trace` and `genesis_trace_z80` sample PC **per m68k frame**, not per instruction: they are NOT true steps (the Z80 executes thousands of instructions between samples). `run_to` is the debugger-style "run to address": a one-shot execute watchpoint that advances frames until the instruction at a bus address executes — the callback fires mid-frame but the state is only read at the END of that frame, so it is a coarse step (one frame each), not an instruction step.
